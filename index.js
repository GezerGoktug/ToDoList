...
class UI {
  ...
  static display() {
    ...
    if (todos.length === 0) {
      ...
      todos.forEach((item) => {
        result += `<div id="todolist"  class="d-flex flex justify-content-between align-items-center p-3" > <div class="w-75">${item.todolistvalue}</div>
        <div class="w-25"><button data-id="${item.todolistid}" value="${item.todolistvalue}" class="btn btn-outline-danger w-100 fs-2 my-2 " onclick="UI.clear(event)">Sil</button></div></div>
        </div>`;
      });
